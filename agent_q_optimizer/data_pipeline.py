"""
Data Pipeline — Institutional State Vector (Context Optimization PRD).

Architecture:
  1. Per-symbol in-memory rolling window (≤300 data points, ~1s cadence = 5 min).
  2. Watcher computes 5 institutional metrics from the window — Z-scores, BPS, adverseselection.
  3. Compressed 5-line State Vector replaces raw floats as LLM input, forcing the
     model to act on normalised statistical anomalies rather than uncontextualised numbers.
...
# Lazy Redis client (shared across calls)
_redis_client = None
...
def _fetch_latest_tick(symbol: str) -> Tuple[float, float, float]:
    ...
    Uses engine_d:{symbol}:pipeline for price/spread and
    telemetry:engine_d:{symbol} for tfi.
    ...
    try:
        ...
        if raw_pipe:
            ...
            # Use actual LOB bid/ask for native spread; fallback to AS model spread
...
def _fetch_adverse_selection(symbol: str, window_minutes: int) -> float:
    ...
    Returns 0.0 if no fills (not being run over if no data).
    """
    try:
...
def get_state_vector(symbol: str, window_minutes: int = 5) -> str:
    ...
    institutional metrics, and returns a compressed 5-line string ready
    for LLM injection.
    """
    sym  = symbol.upper()
    ...
    try:
        if len(hist) >= 2:
            ...
            # A. Micro-Volatility (BPS/sec)
            returns  = np.diff(prices) / (prices[:-1] + 1e-12)
            vol_bps  = float(np.std(returns) * 10_000)

            # B. TFI Z-Score — is current flow a statistical anomaly?
            tfi_zscore = float(
                (tfis[-1] - np.mean(tfis)) / (np.std(tfis) + 1e-9)
            )

            # C. Micro-Trend Drift (total BPS move over window)
            drift_bps = float(
                ((prices[-1] - prices[0]) / (prices[0] + 1e-12)) * 10_000
            )

            # D. Native LOB Spread (average ticks over window)
            native_spread = float(np.mean(spreads))

        # E. Adverse Selection from Postgres (uses full window_minutes)
        adverse_selection_pct = _fetch_adverse_selection(sym, window_minutes)

    except Exception as e:
        log.error("[%s] State vector calculation failed: %s", sym, e)
        vol_bps, tfi_zscore, drift_bps, native_spread, adverse_selection_pct = \
            0.0, 0.0, 0.0, 5.0, 0.0

    # ── 3. Semantic labels (human-readable for LLM) ───────────────────────────
    vol_str   = "EXTREME"           if vol_bps      >  20   else \
                "HIGH"              if vol_bps      >  10   else "NORMAL"
    tox_str   = "TOXIC BUYING"      if tfi_zscore   >   2.0 else \
                "TOXIC DUMPING"     if tfi_zscore   <  -2.0 else "CLEAN"
    trend_str = "STRONG UPTREND"    if drift_bps    >  30   else \
                "STRONG DOWNTREND"  if drift_bps    < -30   else "CHOP/RANGE"
    adv_str   = "DANGER (Run Over)" if adverse_selection_pct > 60 else \
                "SAFE (Capturing Spread)"

    # ── 4. Compressed State Vector (~50 tokens) ───────────────────────────────
    return (
        f"[ORACLE 5M STATE VECTOR - {sym}]\n"
        f"1. Micro-Volatility: {vol_bps:.2f} bps/sec ({vol_str})\n"
        f"2. Order Flow Toxicity: {tfi_zscore:+.2f}σ ({tox_str})\n"
        f"3. Market Drift: {drift_bps:+.2f} bps/5m ({trend_str})\n"
        f"4. Native LOB Spread: {native_spread:.1f} ticks\n"
        f"5. Adverse Selection: {adverse_selection_pct:.1f}% ({adv_str})"
    )
